    ...
    if (telemetry.length >= 1) {
        ...
        for (let k = 0; k < telemetry.length; k++) {
            ...
            for (let i = 0; i < rooms.length; i++) {
                ...
                if (telemetry[k].deviceId === rooms[i].tempDevice) {
                    rooms[i].tempValue = Math.round(telemetry[k].value);
                    rooms[i].time = new Date(telemetry[k].time * 1000).toLocaleTimeString();
                ...
                else if (telemetry[k].deviceId === rooms[i].humidDevice) {
    
                    rooms[i].humidValue = Math.round(telemetry[k].value);
                    rooms[i].time = new Date(telemetry[k].time * 1000).toLocaleTimeString();
